...
{
    const namesList = <ul className={classes.list}>{props.students.map ((student) => <li key={student.username} className={classes.listItem}>{(student.givenName === "" ? student.forename : student.givenName) + " " + student.surname}</li>)}</ul>; 
